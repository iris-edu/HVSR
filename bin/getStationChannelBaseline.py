...
def usage():
   ...
   """
    ...
    print('\n\nexamples:\ngetStationChannelBaseline.py net=IU sta=ANMO loc=00 chan=BHZ start=2002-11-20 '
          'end=2008-11-20 plot=1 plotnnm=1 '
          'verbose=1 percentlow=10 percenthigh=90')
    print('\ngetStationChannelBaseline.py net=TA sta=TCOL loc=-- chan=BHZ,BHN,BHE start=2013-01-01 '
          'end=2014-01-01 plot=1 plotnnm=1 '
          'verbose=1 percentlow=10 percenthigh=90\n\n')
...
channel_list = get_param(args, 'chan', value=param.chan)
...
if station is None:
    ...
    sys.exit()
location = get_param(args, 'loc', value='*')
if location == 'DASH':
    location = '--'
if location is None:
...
percent_low = get_param(args, 'percentlow', value=param.percentlow)
percent_mid = get_param(args, 'percentmid', value=param.percentmid)
percent_high = get_param(args, 'percenthigh', value=param.percenthigh)
percentile = [float(percent_low), float(percent_mid), float(percent_high)]
percent_plot_type = get_param(args, 'percentPlotType', value=param.percentPlotType)
...
x_type = get_param(args, 'xtype', value=param.xtype)
...
for channel in channels:
    ...
    if do_plot > 0 and got_data:
        ...
        if percent_plot_type == 'line':
            plt.plot(np.array(x_values), pct_high, lw=param.lw2, c=param.colorHigh, label='{}%'.format(percent_high))
            plt.plot(np.array(x_values), pct_mid, lw=param.lw2, c=param.colorMid, label='{}%'.format(percent_mid))
            plt.plot(np.array(x_values), pct_low, lw=param.lw2, c=param.colorLow, label='{}%'.format(percent_low))
        else:
        ...
        if plot_nnm:
            ...
            if x_type != 'period':
                nlnm_x = 1.0 / nlnm_x
                nhnm_x = 1.0 / nhnm_x
            plt.plot(nlnm_x, nlnm_y, lw=param.lw2, ls='--', c='k', label='NLNM, NHNM')
            plt.plot(nhnm_x, nhnm_y, lw=param.lw2, ls='--', c='k')
...
if do_plot > 0 and got_data:
    ...
    plt.savefig(plot_fileName, dpi=param.imageDpi, transparent=True, bbox_inches='tight', pad_inches=0.1)
